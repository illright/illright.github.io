...
Я могу в дальнейшем использовать слово "инстанс" (instance), обращаясь к объектам класса. Делается это для того, чтобы отличать объект конкретного класса от просто объекта, а также чтобы для вас привычнее выглядели некоторые сообщения об ошибках в Python.
